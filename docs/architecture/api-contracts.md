...
| `GET /sessions` | セッション一覧を取得する | 同じ bridge プロセス内のメモリ上のセッションを、名前、作成時刻、最終更新時刻、最新メッセージ要約とともに返し、iPhone アプリ再起動後の再接続に使う |
| `GET /sessions/:id` | セッション状態を取得する | HTTP で開始する |
| `PATCH /sessions/:id` | セッション名を更新する | 同じ bridge プロセス内のメモリ上のセッション名を変更する |
...
現時点では `packages/contracts` に `Session`、`Message`、`GET /sessions` のレスポンス、`POST /sessions` のレスポンス、`GET /sessions/:id/messages` のレスポンス、`POST /sessions/:id/prompts` のリクエストとレスポンスを定義する。`Session` は一覧表示と再接続のために、`id`、`name`、`createdAt`、`updatedAt`、`latestMessageSummary` を持つ。セッション名は `PATCH /sessions/:id` で更新できる。
