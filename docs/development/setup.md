...
| 変数名 | 必須 | 用途 | 例 |
| --- | --- | --- | --- |
| `EXPO_SANPO_PROMPT_DRIVER` | 任意 | prompt 実行方式を切り替える | `tmux`, `codex` |
...
この段階では tmux 上に `codex --no-alt-screen` を起動し、prompt を入力して `Escape` + `Enter` を送信し、`capture-pane` の結果を返す。これは Codex CLI 側の Option+Enter 送信設定を tmux から再現するための暫定実装である。`EXPO_SANPO_CODEX_WORKDIR` を指定すると、Codex CLI の起動ディレクトリを明示できる。完了判定は未実装であり、送信後に短く待ってから画面 capture を返す。
