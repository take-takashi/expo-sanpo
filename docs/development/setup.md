...
必要な環境変数を記載する。秘密情報の値は記載しない。開発コマンドで頻繁に切り替える値は、可能な限り `mise run ... --flag` の task 引数で指定する。環境変数は後方互換、外部プロセス設定、または詳細な Irodori-TTS 調整用として扱う。
...
`mise run dev` は Expo CLI を LAN mode で起動する。Mac で Tailscale が有効な場合は、起動時に `tailscale ip -4` から Tailscale IP を取得し、Expo Go に渡す URL の host として自動設定する。これにより、起動時から Tailscale 経由で iPhone が Metro の bundle を取得できる。

Expo CLI は起動後に QR コードや manifest URL の host を動的に切り替えないため、すでに通常の LAN IP で起動している dev server を Tailscale 用の URL に変える場合は、一度 `mise run dev` を再起動する。Tailscale hostname または IP を手動で固定したい場合は、次のように `--host` を指定する。

```sh
mise run dev --host <Mac の Tailscale hostname または IP>
```

host の選択を明示したい場合は `--host-mode` を使う。`auto` は Tailscale IP を優先し、`tailscale` は Tailscale IP が取得できない場合に失敗し、`lan` は Expo CLI の LAN 自動選択を使う。

```sh
mise run dev --host-mode tailscale
mise run dev --host-mode lan
```

Tailscale を使わず、Expo の tunnel 経由で接続する場合は、次のコマンドを使う。
...
```sh
mise run dev:bridge --driver tmux
```
...
```sh
mise run dev:bridge --driver codex
```

この段階では tmux 上に `codex --no-alt-screen` を起動し、prompt を入力して送信キーを送った後、送信前後の `capture-pane` 差分を返す。送信キーはデフォルトで `Escape` + `Enter` とし、Codex CLI 側の Option+Enter 送信設定を tmux から再現する。Mac 側の Codex CLI 設定が異なる場合は、`mise run dev:bridge --driver codex --submit-keys Enter` のように tmux キー列をカンマ区切りで指定する。`--codex-workdir` を指定すると、Codex CLI の起動ディレクトリを明示できる。完了判定は未実装であり、送信後に短く待ってから画面 capture を返す。

Codex app-server の構造化イベント取得を検証する場合は、次のように起動する。

```sh
mise run dev:bridge --driver codex-app-server
```

この driver は `codex app-server --listen stdio://` を子プロセスとして起動し、bridge の sessionId ごとに Codex thread を作成または再利用する。prompt 送信時は `turn/start` を使い、`agentMessage.phase = final_answer` の completed item だけを assistant message として返す。`--codex-workdir` を指定すると、thread と turn の作業ディレクトリを明示できる。turn が完了しない場合は、既定で 10 分後に timeout する。timeout を変える場合は `--turn-timeout-ms` を指定する。
...
このコマンドは、既定では `~/fork/Irodori-TTS` で `uv sync --extra cpu` を実行する。repo の場所が異なる場合は、`mise run tts:irodori:install --dir <path>` を指定する。macOS では `cpu` extra が PyPI の PyTorch wheel を使うため、MPS もこの環境で利用する。
...
高速化の初期値として、`--num-steps 6`、`--t-schedule-mode sway`、`--sway-coeff -1.0` 相当を使う。初回合成の待ち時間を先に吸収したい場合は、`mise run dev:tts:irodori --prewarm-text こんにちは。` のように指定して起動する。
